...
use humhub\modules\user\models\forms\Validation;
use Yii;
...
class AccountController extends BaseAccountController
{
    ...
    /**
     * Bachelor : form for account validation
     */
    public function actionValidate()
    {
        $model = new Validation;
        if($model->load(Yii::$app->request->post()) && $model->validate()) {
            $POST_VARIABLE = Yii::$app->request->post('Validation');
            $model->idUser = Yii::$app->user->id;
            $model->message = $POST_VARIABLE['message'];
            $model->idGroupAsked = $POST_VARIABLE['idGroupAsked'];

            if(!$model->isAlreadyInQueue()){
                $model->save();
                //TODO : add confirmation message
            }
            //TODO : error message saying a request is already posted
